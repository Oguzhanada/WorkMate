...
export default function BecomeProviderPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (data) {
        ...
        // Capture Stripe Connect account ID if available
        if (typeof data.stripe_account_id === 'string' && data.stripe_account_id.trim()) {
  ...
  const handleStripeConnect = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setStripeConnectError(json.error ?? 'Could not create Stripe onboarding link. Please try again.');
        return;
      }
            ...
            {stripeAccountId && (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <CreditCard size={18} style={{ color: 'var(--wm-primary-dark)' }} />
                  <h3
                    className="text-sm font-bold"
                    style={{ fontFamily: 'var(--wm-font-display)', color: 'var(--wm-primary-dark)', margin: 0 }}
                  >
                    Set Up Payments with Stripe
                  </h3>
                </div>
                <p className="text-xs mb-3" style={{ color: 'var(--wm-foreground)' }}>
                  Complete your Stripe onboarding to receive payments directly from customers. You will be redirected to Stripe to verify your identity and add bank details.
                </p>
                {stripeConnectError && (
                  <p className="text-xs mb-2" style={{ color: 'var(--wm-destructive)' }}>
                    {stripeConnectError}
                  </p>
                )}
                <Button
