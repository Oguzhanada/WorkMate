...
function rankProviders(
  ...
  const buckets = new Map<string, Array<ProfileRow & {score: number; rating: number; reviewCount: number}>>();
  ...
  const balanced: Array<ProfileRow & {score: number; rating: number; reviewCount: number}> = [];
  let remaining = true;
...
export async function GET() {
  ...
  const profilesQuery = supabase
    .from('profiles')
    .select('id,full_name,county,avatar_url,created_at')
    .eq('is_verified', true)
  ...
  const ranked = rankProviders(providers, scoreMap);
