...
export async function POST(request: NextRequest) {
  ...
  // ── Helper: mark event as failed in webhook_events ─────────────────────
  async function markEventFailed(errorMessage: string) {
    try {
      await supabase
        .from('webhook_events')
      ...
      // Best-effort — failure tracking should never block the response
    }
  }
  ...
  if (
    ...
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing payment intent event');
    }
  ...
  if (event.type === 'charge.dispute.created') {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing dispute event');
    }
  ...
  if (event.type === 'account.updated') {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing account update');
    }
  ...
  if (event.type === 'customer.subscription.created') {
    try {
      ...
      if (!parsed.success) {
        await markEventFailed(`Parse error: ${parsed.error.issues.map(i => i.message).join(', ')}`);
        return NextResponse.json({ received: true }, { status: 200 });
      }
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing subscription.created');
    }
  ...
  if (event.type === 'customer.subscription.updated') {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing subscription.updated');
    }
  ...
  if (event.type === 'customer.subscription.deleted') {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing subscription.deleted');
    }
  ...
  if (event.type === 'invoice.payment_failed') {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing invoice.payment_failed');
    }
  ...
  if (event.type === 'invoice.paid' || event.type === 'invoice.payment_succeeded') {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing invoice.paid');
    }
  ...
  if (event.type.startsWith('identity.verification_session')) {
    try {
      ...
      await markEventFailed(err instanceof Error ? err.message : 'Unknown error processing identity event');
    }
