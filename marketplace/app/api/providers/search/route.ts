...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `provider-search:${JSON.stringify({ q, category_id, county, verified_only, sort, page, limit })}`;

  // Cache search results for 90 seconds — public read-only data.
  type ProviderResult = {
    id: string;
    full_name: string;
  ...
  try {
    const payload = await cacheGet<SearchPayload>(cacheKey, async () => {
    ...
    return { providers: results, total, page, limit };
  }, 90);
