...
function KpiCard({
  ...
  const card = (
    <div style={{
      ...
      <div>
        <p style={{ margin: '0 0 2px', fontSize: '28px', fontWeight: 800, color: T.text, lineHeight: 1, fontFamily: 'var(--wm-font-display)' }}>
          {value.toLocaleString()}
        </p>
        <p style={{ margin: 0, fontSize: '12px', fontWeight: 500, color: T.muted }}>{label}</p>
      </div>
      {sub && (
        <p style={{ margin: 0, fontSize: '11px', color: urgent && value > 0 ? T.amber : T.muted }}>
          {sub}
        </p>
      )}
    </div>
  ...
  if (href) {
    return <Link href={`/${locale}${href}`} style={{ textDecoration: 'none' }}>{card}</Link>;
  }
  return card;
}

// ─── Section header ───────────────────────────────────────────────────────────
function SectionHeader({ title, linkLabel, linkHref, locale }: { title: string; linkLabel?: string; linkHref?: string; locale: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
      <h2 style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: T.text, fontFamily: 'var(--wm-font-display)' }}>{title}</h2>
      {linkLabel && linkHref && (
        <Link href={`/${locale}${linkHref}`} style={{ display: 'flex', alignItems: 'center', gap: '3px', fontSize: '11px', fontWeight: 600, color: T.emerald, textDecoration: 'none' }}>
...
function ActionBadge({ action }: { action: string }) {
  ...
  return (
    <span style={{ fontSize: '10px', fontWeight: 600, color: style.color, background: style.bg, borderRadius: '6px', padding: '2px 7px', textTransform: 'capitalize', whiteSpace: 'nowrap' }}>
      {action.replace(/_/g, ' ')}
    </span>
  );
}
...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: '8px', padding: '8px 12px', fontSize: '12px', boxShadow: T.cardShadow }}>
      <p style={{ margin: 0, color: T.muted }}>{label}</p>
      <p style={{ margin: '2px 0 0', fontWeight: 700, color: T.text }}>{payload[0].value}</p>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: '24px', background: T.contentBg, minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        {data.pendingVerification > 0 && (
          <Link href={`/${locale}/dashboard/admin/applications`} style={{
            ...
            {data.pendingVerification} pending application{data.pendingVerification > 1 ? 's' : ''}
          </Link>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
        <KpiCard
          icon={Users} iconColor={T.emerald} iconBg={T.emeraldBg}
          label="Total Users" value={totalUsers}
          sub={`${data.pros} providers · ${data.customers} customers`}
          locale={locale}
        />
        <KpiCard
          icon={TrendingUp} iconColor={T.violet} iconBg={T.violetBg}
          label="New This Week" value={data.newUsers7d}
          sub="Registered in last 7 days"
          locale={locale}
        />
        <KpiCard
          icon={Briefcase} iconColor={T.sky} iconBg={T.skyBg}
          label="Total Jobs" value={totalJobs}
          sub={`${data.openJobs} open · ${data.activeJobs} active`}
          locale={locale}
        />
        <KpiCard
          icon={CreditCard} iconColor={T.amber} iconBg={T.amberBg}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
        <KpiCard
          icon={UserCheck} iconColor={T.amber} iconBg={T.amberBg}
          label="Pending Verification" value={data.pendingVerification}
          sub="ID applications to review" urgent
          href="/dashboard/admin/applications" locale={locale}
        />
        <KpiCard
          icon={UserCheck} iconColor={T.sky} iconBg={T.skyBg}
          label="Pending Documents" value={data.pendingDocs}
          sub="Documents awaiting review"
          href="/dashboard/admin/verification" locale={locale}
        />
        <KpiCard
          icon={AlertTriangle} iconColor={T.rose} iconBg={T.roseBg}
          label="Rejected IDs" value={data.rejectedVerification}
          sub="Failed identity checks"
          locale={locale}
        />
        <KpiCard
          icon={Users} iconColor={T.navy} iconBg={T.navyBg}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
        ...
        <div style={{ background: T.cardBg, border: `1.5px solid ${T.cardBorder}`, borderRadius: T.cardRadius, boxShadow: T.cardShadow, padding: '20px' }}>
          <SectionHeader title="User Registrations — Last 7 Days" locale={locale} />
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={data.weeklyRegistrations} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <defs>
              ...
              <YAxis allowDecimals={false} tick={{ fontSize: 10, fill: T.muted }} axisLine={false} tickLine={false} />
              <Tooltip content={<ChartTooltip />} />
              <Area type="monotone" dataKey="count" stroke={T.emerald} strokeWidth={2} fill="url(#regGrad)" dot={{ fill: T.emerald, r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ background: T.cardBg, border: `1.5px solid ${T.cardBorder}`, borderRadius: T.cardRadius, boxShadow: T.cardShadow, padding: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={jobChartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {data.recentAuditLogs.map((log, i) => (
                <div key={i} style={{
                  ...
                  padding: '9px 0',
                  borderBottom: i < data.recentAuditLogs.length - 1 ? `1px solid ${T.border}` : 'none',
                }}>
                  <ActionBadge action={log.action} />
                  <span style={{ fontSize: '11px', color: T.muted, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ background: T.cardBg, border: `1.5px solid ${T.cardBorder}`, borderRadius: T.cardRadius, boxShadow: T.cardShadow, padding: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px' }}>
            <span style={{ fontSize: '11px', fontWeight: 600, color: T.emerald, background: T.emeraldBg, borderRadius: '6px', padding: '2px 8px' }}>
              {enabledFlags} on
            </span>
            <span style={{ fontSize: '11px', color: T.muted }}>/ {data.featureFlags.length} total</span>
          </div>
          {data.featureFlags.length === 0 ? (
            <p style={{ fontSize: '12px', color: T.muted, margin: 0 }}>No flags configured</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {data.featureFlags.slice(0, 6).map((flag, i) => (
                <div key={flag.flag_key} style={{
                  ...
                  padding: '7px 0',
                  borderBottom: i < Math.min(data.featureFlags.length, 6) - 1 ? `1px solid ${T.border}` : 'none',
                }}>
                  <span style={{ fontSize: '11px', color: T.text, fontFamily: 'var(--wm-font-mono)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, marginRight: '8px' }}>
                    {flag.flag_key}
                  </span>
                  <span style={{
                    fontSize: '10px', fontWeight: 700, borderRadius: '5px', padding: '2px 7px', flexShrink: 0,
        ...
        <div style={{ background: T.cardBg, border: `1.5px solid ${T.cardBorder}`, borderRadius: T.cardRadius, boxShadow: T.cardShadow, padding: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {[
              ...
              <Link key={a.href} href={`/${locale}${a.href}`} style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                padding: '9px 12px',
                borderRadius: '8px',
                ...
                <span style={{ fontSize: '12px', fontWeight: 500, color: a.urgent && a.count > 0 ? T.amber : T.text }}>
                  {a.label}
                </span>
                {a.count > 0 ? (
                  <span style={{ background: a.urgent ? T.amber : T.emerald, color: '#fff', borderRadius: '10px', padding: '1px 8px', fontSize: '10px', fontWeight: 700 }}>
                    {a.count}
                  </span>
                ) : (
