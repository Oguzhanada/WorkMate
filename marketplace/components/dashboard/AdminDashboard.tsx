...
export default function AdminDashboard({ data, locale }: { data: AdminDashboardData; locale: string }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--wm-bg)' }}>
      {/* Sidebar */}
      <aside style={{
        width: '200px', flexShrink: 0, background: 'var(--wm-surface)',
        borderRight: '1.5px solid var(--wm-border)', padding: '20px 0',
        position: 'sticky', top: 0, height: '100vh', overflowY: 'auto',
      }}>
        <div style={{ padding: '0 16px 14px', borderBottom: '1px solid var(--wm-border)', marginBottom: '8px' }}>
          <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--wm-muted)', margin: '0 0 2px' }}>Admin Panel</p>
          <p style={{ fontSize: '12px', color: 'var(--wm-foreground)', margin: 0, fontWeight: 600 }}>WorkMate HQ</p>
        </div>
        {NAV_ITEMS.map((item) => {
          const isActive = item.path === '/dashboard/admin';
          const badge = item.path === '/dashboard/admin/applications' ? data.pendingVerification
            : item.path === '/dashboard/admin/verification' ? data.pendingDocs
            : 0;
          return (
            <Link key={item.path} href={`/${locale}${item.path}`} style={{
              ...
              {item.label}
              {badge > 0 && (
                <span style={{ marginLeft: 'auto', background: 'var(--wm-gold)', color: 'var(--wm-navy)', borderRadius: '10px', padding: '1px 7px', fontSize: '10px', fontWeight: 700 }}>
                  {badge}
                </span>
              )}
            </Link>
